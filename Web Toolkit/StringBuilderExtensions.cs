...
namespace NeoSmart.Web
{
    public static class StringBuilderExtensions
    {
        ...
        public static StringBuilder TrimEnd(this StringBuilder sb, char[] values)
        {
            var trimLookup = new HashSet<char>(values);
            for (int i = sb.Length - 1; i >= 0; --i)
            {
                if (!trimLookup.Contains(sb[i]))
                {
                    sb.Remove(i, sb.Length - i - 1);
                    break;
                }
            }
        ...
        {
            return test.Length <= sb.Length && !test.Where((t, i) => sb[sb.Length - 1 - i] != t).Any();
        }
